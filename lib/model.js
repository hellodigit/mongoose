...
function handleSave (promise, self) {
  // Capture stack trace before doing anything asynchronous to ensure
  // we have a useful stack trace for where Model.save() originated.
  var originalError = new Error();

  var modifiedPaths = self.modifiedPaths();
  ...
  return tick(function handleSave (err, result) {
    ...
    if (self.$__.version && !self.$__.inserting) {
      ...
      if (numAffected > 0) {
        ...
        var err = new VersionError(self, version, modifiedPaths);

        // ensure we have a useful stack trace for where Model.save() originated
        // that caused VersionError
...
function operand (self, where, delta, data, val, op) {
  ...
  var id = this._doc && this._doc._id;

  if ('$push' == op || '$pushAll' == op || '$addToSet' == op) {
    console.log('Mongoose version case 1: VERSION_INC triggered on id "%s" with op "%s" on path "%s"', id, op, data.path);
    self.$__.version = VERSION_INC;
  }
  else if (/^\$p/.test(op)) {
    console.log('Mongoose version case 2: VERSION_ALL triggered on id "%s" with op "%s" on path "%s"', id, op, data.path);
    // potentially changing array positions
    self.increment();
  }
  else if (Array.isArray(val)) {
    console.log('Mongoose version case 3: VERSION_ALL triggered on id "%s" with op "%s" on path "%s"', id, op, data.path);
    // $set an array
    self.increment();
  }
  // now handling $set, $unset
    ...
    // subpath of array
    console.log('Mongoose version case 4: VERSION_WHERE triggered on id "%s" with op "%s" on path "%s"', id, op, data.path);
...
Model.prototype.increment = function increment() {
  var id = this._doc && this._doc._id;
  console.log('Mongoose: Model.increment() called on id "%s"', id);
