...
function operand (self, where, delta, data, val, op) {
  ...
  if ('$push' == op || '$pushAll' == op || '$addToSet' == op) {
    console.log('Mongoose version case 1: VERSION_INC triggered with op "%s" on path "%s"', op, data.path);
    self.$__.version = VERSION_INC;
  }
  else if (/^\$p/.test(op)) {
    console.log('Mongoose version case 2: VERSION_ALL triggered with op "%s" on path "%s"', op, data.path);
    // potentially changing array positions
    self.increment();
  }
  else if (Array.isArray(val)) {
    console.log('Mongoose version case 3: VERSION_ALL triggered with op "%s" on path "%s"', op, data.path);
    // $set an array
    self.increment();
  }
  // now handling $set, $unset
    ...
    // subpath of array
    console.log('Mongoose version case 4: VERSION_WHERE triggered with op "%s" on path "%s"', op, data.path);
